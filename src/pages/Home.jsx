...
export default function Home() {
  ...
  const availableVolumes = [...new Set(perfumes.map(p => p.volume).filter(Boolean))].sort((a, b) => a - b)
  ...
  perfumes.forEach(p => {
    if (p.brand) {
      ...
      if (!brandMap.has(lowerBrand)) brandMap.set(lowerBrand, cleanBrand);
    }
  });
      ...
      <main className="flex-1 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 w-full mt-4 sm:mt-8 mb-10 flex flex-col md:flex-row gap-6 items-start">
        ...
        <div className="w-full md:hidden flex flex-col gap-3 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex bg-gray-100 p-1 rounded-xl w-full mb-2">
            <button onClick={() => setPriceType('wholesale')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${priceType === 'wholesale' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
              {t.wholesale}
            </button>
            <button onClick={() => setPriceType('retail')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${priceType === 'retail' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
              {t.retail}
            </button>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setIsFilterMobileOpen(true)} className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2 rounded-xl text-sm font-bold">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>
              {lang === 'kz' ? 'Фильтр' : 'Фильтры'}
            </button>
          </div>
          
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-700 text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 block w-full p-2.5 outline-none font-medium">
          ...
          <div className="h-full md:h-auto overflow-y-auto bg-white md:rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col gap-6 md:sticky md:top-24">
            ...
            <div>
              ...
              <div className="flex flex-col gap-2.5">
                {[{ id: 'all', label: t.all }, { id: 'Мужской', label: t.men }, { id: 'Женский', label: t.women }, { id: 'Унисекс', label: t.unisex }].map(opt => (
                  <label key={opt.id} className="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="gender" checked={genderFilter === opt.id} onChange={() => setGenderFilter(opt.id)} className="w-4.5 h-4.5 border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
                ...
                <div className="flex flex-col gap-2.5 max-h-56 overflow-y-auto pr-2 scrollbar-thin">
                  {availableBrands.map(brand => (
                    <label key={brand} className="flex items-center gap-3 cursor-pointer group">
                      <input type="checkbox" checked={selectedBrands.includes(brand)} onChange={() => toggleBrand(brand)} className="w-4.5 h-4.5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 transition-all cursor-pointer" />
            ...
            <div>
              ...
              <div className="flex flex-col gap-2.5">
                {[{ id: 'all', label: t.all }, { id: 'range1', label: t.range1 }, { id: 'range2', label: t.range2 }, { id: 'range3', label: t.range3 }].map(opt => (
                  <label key={opt.id} className="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="price" checked={priceFilter === opt.id} onChange={() => setPriceFilter(opt.id)} className="w-4.5 h-4.5 border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
                ...
                <div className="flex flex-col gap-2.5 max-h-48 overflow-y-auto pr-2 scrollbar-thin">
                  {availableVolumes.map(vol => (
                    <label key={vol} className="flex items-center gap-3 cursor-pointer group">
                      <input type="checkbox" checked={selectedVolumes.includes(vol)} onChange={() => toggleVolume(vol)} className="w-4.5 h-4.5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
        ...
        <div className="flex-1 w-full">
          <div className="hidden md:flex bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-6 justify-between items-center gap-4">
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl mx-auto">
              <button onClick={() => setPriceType('wholesale')} className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${priceType === 'wholesale' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
                {t.wholesale}
              </button>
              <button onClick={() => setPriceType('retail')} className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${priceType === 'retail' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
                {t.retail}
              </button>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-700 text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 block p-2.5 outline-none font-medium cursor-pointer">
                <option value="new">{lang === 'kz' ? 'Жаңа қосылғандар' : 'Сначала новые'}</option>
                <option value="alphabetical">{t.sortAlphabetical}</option>
                <option value="price-asc">{t.sortCheap}</option>
                <option value="price-desc">{t.sortExpensive}</option>
                ...
                {[...Array(8)].map((_, index) => (
                  <div key={index} className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm flex flex-col h-full animate-pulse">
                    ...
                    <div className="p-3 sm:p-4 flex flex-col flex-grow">
                      ...
                      <div className="mt-auto"><div className="h-6 bg-gray-200 rounded-full w-1/2"></div></div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <>
                ...
                {visibleCount < sortedPerfumes.length && (
                  <div className="mt-8 flex justify-center">
                    <button onClick={() => setVisibleCount(prev => prev + 20)} className="bg-white border border-gray-200 text-indigo-600 font-bold py-3.5 px-8 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">
                      {t.loadMore}
                    </button>
                  </div>
                )}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-end sm:items-center z-[60] p-0 sm:p-4 transition-all">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col sm:flex-row shadow-2xl relative">
            ...
            <div className="w-full sm:w-1/2 bg-gray-50 aspect-square sm:aspect-auto">
              <img src={selectedPerfume.image_url} alt={selectedPerfume.name} loading="lazy" decoding="async" className="w-full h-full object-cover" />
            </div>
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {similarPerfumes.map(sim => (
                      <div key={sim.id} onClick={() => setSelectedPerfume(sim)} className="cursor-pointer group flex flex-col">
                        <div className="aspect-[4/5] rounded-xl overflow-hidden bg-gray-50 mb-2">
                          <img src={sim.image_url} alt={sim.name} loading="lazy" decoding="async" className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                        </div>
              ...
              {cart.length === 0 ? (
                ...
                cart.map(item => (
                  <div key={item.id} className="flex gap-4 border-b border-gray-50 pb-4">
                    <img src={item.image_url} alt={item.name} loading="lazy" decoding="async" className="w-20 h-20 object-cover rounded-xl bg-gray-50" />
